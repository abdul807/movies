...
export function EditEntryForm({ entry, onSuccess }: EditEntryFormProps) {
  ...
  const [imagePreview, setImagePreview] = useState<string | null>(entry.poster_url || null);
  ...
  const mutation = useMutation({
    mutationFn: async (data: EntryInput) => {
      const result = await updateEntry(entry.id, data, imageFile || undefined);
      return result;
    },
    onSuccess: () => {
